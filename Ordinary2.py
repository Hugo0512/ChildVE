...
def ordinary(totalFrameNumber,board_datas,head_datas,eye_datas,lenlet_datas):
                ...
                if tempname==(str(c)+'_eye1.jpg') or tempname== (str(c)+'_eye2.jpg'):#找到了眼睛图片的名字
                    ...
                    #     continue;
                            ...
                            if (xmin + 0.5 * lenswidth) > imagewidth * 0.5:
                                ...
                                # 把板子上的画截出来存放在另外一个文件夹�?
                                originalimage = Image.open("./frame/" + str(c) + ".jpg")  # 之前的完整图
                                picturelastposition=(picturelastposition[0],picturelastposition[1],picturelastposition[0]+picturelastposition[2],picturelastposition[1]+picturelastposition[3])
                                picture = originalimage.crop(picturelastposition)
                                picture.save("./last/lastpicture" + ".jpg")
                                # 此处调用caffe分类
                                IMAGE_FILE=osp.abspath(osp.join(osp.dirname(__file__)))
        ...
        if numpy:
            ...
            if numpy:
                ...
                if numpy:
                    ...
                    result5=ordinary(totalFrameNumber,board_datas,head_datas,eye_datas,eyelet_datas)
                    if result5=='0':
